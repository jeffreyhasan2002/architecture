...
const NAV = [
  ...
  { href: '/how-it-works/gear', label: 'How It Works' },
    ...
    <div style={{ minHeight: '100vh', background: 'var(--color-bg)', color: 'var(--color-ink)' }}>
      ...
      <header style={{ position: 'sticky', top: 0, zIndex: 100, height: '60px', display: 'flex', alignItems: 'center', padding: '0 24px', gap: '16px', background: 'var(--color-bg)', borderBottom: '1px solid var(--color-hairline)' }}>
        ...
        <nav style={{ display: 'flex', gap: '2px' }}>
          {NAV.map(l => (
            <Link key={l.href} href={l.href} style={{ fontFamily: 'var(--font-space-grotesk)', fontWeight: 600, fontSize: '12px', color: l.href === '/how-it-works/gear' ? 'var(--color-accent)' : 'var(--color-muted)', textDecoration: 'none', padding: '5px 10px', borderRadius: '6px', background: l.href === '/how-it-works/gear' ? 'rgba(15,98,254,0.08)' : 'none' }}>{l.label}</Link>
          ))}
        </nav>
      </header>

      {/* Hero */}
      <div style={{ background: 'linear-gradient(135deg, rgba(109,91,208,0.06) 0%, rgba(15,98,254,0.06) 100%)', borderBottom: '1px solid var(--color-hairline)', padding: '48px 24px 40px', textAlign: 'center' }}>
